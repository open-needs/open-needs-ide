...
from pygls.features import (COMPLETION, COMPLETION_ITEM_RESOLVE, TEXT_DOCUMENT_DID_CHANGE,
                            TEXT_DOCUMENT_DID_CLOSE, TEXT_DOCUMENT_DID_OPEN,
                            INITIALIZED, WORKSPACE_DID_CHANGE_CONFIGURATION,
                            DEFINITION, HOVER)
from pygls.server import LanguageServer
from pygls.types import (CompletionItem, CompletionList, CompletionParams,
                         ConfigurationItem, ConfigurationParams,
                         DidChangeTextDocumentParams, Location,
                         DidCloseTextDocumentParams, DidOpenTextDocumentParams,
                         Position, Range, Hover,
                         TextEdit, InitializeParams,
                         CompletionItemKind, InsertTextFormat,
                         MarkupContent, MarkupKind, MessageType)
...
def get_need_type_and_id(ls, params) -> Tuple[str, str]:
    ...
    for key, need in ls.needs_store.needs.items():
        if need['id'] in word:
...
def complete_need_link(ls, params: CompletionParams, lines: List[str], line: str, word: str):
    ...
    if word.count('>') == 2:
        ...
        if requested_type in ls.needs_store.types:
            return CompletionList(True, doc_completion_items(
                ls,
                ls.needs_store.docs_per_type[requested_type],
                word_parts[2]))
                ...
                CompletionItem(
                    ...
                    additional_text_edits=[TextEdit(
                        range=Range(
                            ...
                            end=Position(line_number, start_char + len(substitution))
                        ),
                        new_text='')])
...
def generate_hash(user_name, doc_uri, need_prefix, line_number):
    ...
    return blake2b(f'{user_name}{doc_uri}{need_prefix}{line_number}'.encode(), digest_size=4, salt=salt).hexdigest()
...
async def did_change_workspace_config(ls, params):
    ...
    try:
        ...
        ls.show_message('using needs in (from ws change): ' + os.path.abspath(config[0].needs_file))
...
def update_settings(ls, *args):
    ...
    try:
        ...
        ls.show_message('Error setting document root! Are your settings correct?', msg_type=MessageType.Error)
        return
    try:
        ls.needs_store.load_needs(needs_file)
    except ValueError:
        ls.show_message('Error loading needs.json! Are your settings correct?', msg_type=MessageType.Error)
        return
